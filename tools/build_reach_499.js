// August reach campaign, 499 price tier: AI_Apollo_reach_499_0108.
// Cities: Kyiv (oblast region), Lviv, Vinnytsia. Per city x per video
// (AP_01/02/03, 499 versions) = a group; one adaptive video ad per group
// (9x16 -> story/reels, 4x4 -> feed), unique text per video + 499 offer,
// per-city club link. OUTCOME_AWARENESS/REACH, IG+Threads 18-55. PAUSED, 01.08.
// Usage: node tools/build_reach_499.js test | build
const fs = require('fs');
...
const NEW_CAMP_NAME = 'AI_Apollo_reach_499_0108';
...
const OFFER = '\n\n🎁 Рік спорту — 499 ₴ кожні 4 тижні. Фіксуй ціну на рік. Акція діє з 01.08 до 31.08.2026.';
const VIDEOS = [
  { key: 'AP01', story: 'AP_01_9x16_310826.mp4', feed: 'AP_01_4x4_310826.mp4',
    headline: 'Тут ти — просто ти',
    body: 'Робота. Родина. Дорога. За день ти встигаєш побути всіма — окрім себе.\n\nAPOLLO NEXT — простір, де можна зняти всі ролі. Тут ти нікому нічого не винен: лише рух, лише дихання, лише ти.\n#титвоямета' + OFFER },
  { key: 'AP02', story: 'AP_02_9x16_310826.mp4', feed: 'AP_02_4x4_310826.mp4',
  ...
  { key: 'AP03', story: 'AP_03_9x16_310826.mp4', feed: 'AP_03_4x4_310826.mp4',
...
// Kyiv targets the oblast region (as current Kyiv reach); others target the city.
const CITIES = [
  { name: 'Kyiv', geo: { regions: [{ key: '4290', country: 'UA' }], location_types: ['home', 'recent'] }, budget: 3000, link: 'https://apollo.online/clubs/kyyiv/' },
  { name: 'Lviv', geo: { cities: [{ country: 'UA', distance_unit: 'mile', key: '2378495' }], location_types: ['home', 'recent'] }, budget: 1000, link: 'https://apollo.online/clubs/lviv/' },
  { name: 'Vinnytsia', geo: { cities: [{ country: 'UA', distance_unit: 'mile', key: '2397330' }], location_types: ['home', 'recent'] }, budget: 500, link: 'https://apollo.online/clubs/apollo-next-033-tcz-magigrand/' },
];
...
function targeting(geo) {
  return {
    age_min: 18, age_max: 55, geo_locations: geo,
    publisher_platforms: ['instagram', 'threads'],
    instagram_positions: ['stream', 'story', 'explore', 'reels', 'explore_home', 'profile_feed'],
    threads_positions: ['threads_stream'], targeting_automation: { advantage_audience: 0 },
  };
}
async function createAdset(campaignId, name, budget, geo) {
  const r = await apiPost(`${ACCT}/adsets`, {
    name, campaign_id: campaignId, optimization_goal: 'REACH', billing_event: 'IMPRESSIONS', bid_strategy: 'LOWEST_COST_WITHOUT_CAP',
    daily_budget: String(budget), promoted_object: JSON.stringify({ page_id: PAGE_ID }),
    targeting: JSON.stringify(targeting(geo)), start_time: START_TIME, status: 'PAUSED',
  });
...
async function prepVideo(v) {
  ...
  v.storyThumb = await uploadImage(thumb(v.story, `${v.key}499_st.jpg`), `${v.key}499_st.jpg`);
  v.feedThumb = await uploadImage(thumb(v.feed, `${v.key}499_fd.jpg`), `${v.key}499_fd.jpg`);
  console.log(`     story ${v.storyVid} / feed ${v.feedVid} | thumbs ok`);
}

async function test() {
  const v = VIDEOS[1]; // AP02
  await prepVideo(v);
  const cr = await createCreative(creativePayload(v, v.key + '_TEST', CITIES[0].link));
  console.log('  creative', cr);
  const camp = await apiPost(`${ACCT}/campaigns`, { name: 'ZZZ_REACH499_TEST_DEL', objective: 'OUTCOME_AWARENESS', buying_type: 'AUCTION', special_ad_categories: JSON.stringify([]), is_adset_budget_sharing_enabled: 'false', status: 'PAUSED' });
  try {
    const asId = await createAdset(camp.id, 'ZZZ_Kyiv_AP02', 3000, CITIES[0].geo);
    const adId = await createAd(asId, 'AP02', cr);
...
async function build() {
  ...
  for (const c of CITIES) {
    for (const v of VIDEOS) {
      const name = `${c.name}_18-55_${v.key}`;
      const cr = await createCreative(creativePayload(v, `${v.key}_${c.name}`, c.link));
      const asId = await createAdset(camp.id, name, c.budget, c.geo);
      await createAd(asId, v.key, cr);
      log.ads_count++; log.adsets.push({ name, id: asId });
    }
    console.log(`  📂 ${c.name}: 3 groups (AP01/02/03) @ $${c.budget / 100}/d | link ${c.link}`);
  }
