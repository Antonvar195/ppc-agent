...
Правила:
- ⚠️ КРИТИЧНО: БУДЬ-ЯКА кампанія, створена через агента (Claude Code / Telegram),
  ЗАВЖДИ починається з **AI_** (`AI_Apollo_...`). Префікс `A_` — ТІЛЬКИ для ручного
  створення людиною. Не копіюй `A_` зі старих кампаній — заміняй на `AI_`.
...
✅ Threads ВИКОРИСТОВУЄТЬСЯ в конверсійних кампаніях.
Заміряно 29.07.2026 за 30 днів на живому акаунті:

| Плейсмент | Витрати | CTR | Покупок | CPA |
|-----------|---------|-----|---------|-----|
| Threads   | $879    | 0.44% | 25    | **$35** |
| Facebook  | $3 080  | 1.14% | 60    | $51 |
| Instagram | $25 975 | 0.78% | 419   | $62 |

Threads має найгірший CTR і найкращий CPA — судити цей плейсмент
по CTR не можна.

Старе обмеження ("несумісний з більшістю цілей") більше не діє —
auto-fix, що вирізав Threads, прибрано з `tools/create_campaign.js`.

⚠️ Вибірка поки невелика ($879, 25 покупок). Перевірити повторно
після масштабування.
